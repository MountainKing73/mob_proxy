...
RUN cargo zigbuild -r \
    --target x86_64-unknown-linux-musl --target aarch64-unknown-linux-musl && \
  mkdir /app/linux && \
  cp target/aarch64-unknown-linux-musl/release/mob_proxy /app/linux/arm64 && \
  cp target/x86_64-unknown-linux-musl/release/mob_proxy /app/linux/amd64
...
WORKDIR /app
ARG TARGETPLATFORM
COPY --from=builder /app/${TARGETPLATFORM} /app/mob_proxy
CMD ["/app/mob_proxy"]
